...
@Tag(
    ...
    description = "Endpoints for merchant profile, dashboard and administration"
)
public class MerchantController {
    ...
    @Operation(
        summary = "Get merchant dashboard with active subscription, coupon count and redemption stats"
    )
    ...
    @Operation(
        summary = "Internal sync endpoint: create merchant profile synced from auth-service"
    )
    public ResponseEntity<MerchantProfileDto> createInternalMerchant(
            @RequestBody MerchantProfileDto dto) {

        return new ResponseEntity<>(
                merchantService.createInternalMerchant(dto),
                HttpStatus.CREATED
        );
    }
    ...
    @Operation(
        summary = "Update merchant status (PENDING, APPROVED, REJECTED, SUSPENDED) - Called by Admin"
    )
