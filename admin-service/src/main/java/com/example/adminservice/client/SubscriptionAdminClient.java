...
public interface SubscriptionAdminClient {
	...
	default SubscriptionResponseDto getSubscriptionByIdFallback(Long id, Throwable t) {
		LoggerFactory.getLogger(SubscriptionAdminClient.class).error(
				"Circuit breaker fallback: subscription-service unavailable for getSubscriptionById {}. Reason: {}", id,
				t.getMessage());
		throw new com.example.adminservice.exception.BadRequestException(
				"Subscription service is temporarily unavailable.");
	}

	default SubscriptionPlanResponseDto createSubscriptionPlanFallback(CreateSubscriptionPlanRequestDto request,
			Throwable t) {
		LoggerFactory.getLogger(SubscriptionAdminClient.class).error(
				"Circuit breaker fallback: subscription-service unavailable for createSubscriptionPlan. Reason: {}",
				t.getMessage());
		throw new com.example.adminservice.exception.BadRequestException(
				"Subscription service is temporarily unavailable. Could not create plan.");
	}
}
