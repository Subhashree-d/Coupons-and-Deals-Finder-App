...
public class AdminService {
	...
	public AdminService(MerchantAdminClient merchantAdminClient, SubscriptionAdminClient subscriptionAdminClient,
			CouponAdminClient couponAdminClient, PaymentAdminClient paymentAdminClient,
	...
	public MerchantResponseDto approveMerchant(Long id, String adminEmail) {
		...
		auditLogRepository.save(new AdminAuditLog(null, "APPROVE_MERCHANT", "MERCHANT", id,
				adminEmail != null ? adminEmail : "ADMIN", "Merchant approved"));
		return res;
	}
	...
	public MerchantResponseDto rejectMerchant(Long id, String adminEmail) {
		...
		auditLogRepository.save(new AdminAuditLog(null, "REJECT_MERCHANT", "MERCHANT", id,
				adminEmail != null ? adminEmail : "ADMIN", "Merchant rejected"));
		return res;
	}
	...
	public MerchantResponseDto suspendMerchant(Long id, String adminEmail) {
		...
		auditLogRepository.save(new AdminAuditLog(null, "SUSPEND_MERCHANT", "MERCHANT", id,
				adminEmail != null ? adminEmail : "ADMIN", "Merchant suspended"));
		return res;
	}
	...
	public CouponResponseDto approveCoupon(Long id, String adminEmail) {
		...
		auditLogRepository.save(new AdminAuditLog(null, "APPROVE_COUPON", "COUPON", id,
				adminEmail != null ? adminEmail : "ADMIN", "Coupon approved"));
		return res;
	}
	...
	public CouponResponseDto rejectCoupon(Long id, String adminEmail) {
		...
		auditLogRepository.save(new AdminAuditLog(null, "REJECT_COUPON", "COUPON", id,
				adminEmail != null ? adminEmail : "ADMIN", "Coupon rejected"));
		return res;
	}
	...
	public PaymentResponseDto verifyPayment(Long id, String adminEmail) {
		...
		auditLogRepository.save(new AdminAuditLog(null, "VERIFY_PAYMENT", "PAYMENT", id,
				adminEmail != null ? adminEmail : "ADMIN", "Payment verified and activated"));
		return res;
	}
	...
	public PlatformMerchantRevenueResponseDto getMerchantRevenueSummary(LocalDate fromDate, LocalDate toDate) {
		...
		Map<Long, MerchantRevenueResponseDto> revenueMap = (revenueList != null)
				? revenueList.stream().collect(
						Collectors.toMap(MerchantRevenueResponseDto::getMerchantId, Function.identity(), (a, b) -> a))
		...
		if (allMerchants != null) {
			for (MerchantResponseDto merchant : allMerchants) {
				...
				merchantItems.add(new MerchantRevenueItemDto(merchantId,
						merchant.getBusinessName() != null ? merchant.getBusinessName() : "Merchant " + merchantId,
						totalRevenue, successfulPayments));
			}
		}

		// Include any remaining merchants from payment records if not present in
		// allMerchants
		if (revenueList != null) {
			for (MerchantRevenueResponseDto rev : revenueList) {
				if (!processedMerchantIds.contains(rev.getMerchantId())) {
					merchantItems.add(new MerchantRevenueItemDto(rev.getMerchantId(), "Merchant " + rev.getMerchantId(),
							rev.getTotalRevenue() != null ? rev.getTotalRevenue().setScale(2, RoundingMode.HALF_UP)
									: BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP),
		...
		BigDecimal totalPlatformRevenue = merchantItems.stream().map(MerchantRevenueItemDto::getTotalRevenue)
				.reduce(BigDecimal.ZERO, BigDecimal::add).setScale(2, RoundingMode.HALF_UP);

		return new PlatformMerchantRevenueResponseDto(totalPlatformRevenue, "INR", merchantItems);
	}

	public SpecificMerchantRevenueResponseDto getSpecificMerchantRevenue(Long merchantId, LocalDate fromDate,
			LocalDate toDate) {
		validateDateRange(fromDate, toDate);
		log.info("Admin fetching revenue for specific merchant ID: {} with fromDate: {}, toDate: {}", merchantId,
				fromDate, toDate);
		...
		BigDecimal totalRevenue = (rev != null && rev.getTotalRevenue() != null)
				? rev.getTotalRevenue().setScale(2, RoundingMode.HALF_UP)
				: BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP);
		Long successfulPayments = (rev != null && rev.getSuccessfulPayments() != null) ? rev.getSuccessfulPayments()
				: 0L;

		return new SpecificMerchantRevenueResponseDto(merchantId,
				merchant.getBusinessName() != null ? merchant.getBusinessName() : "Merchant " + merchantId,
				totalRevenue, successfulPayments, "INR");
	}

	public SubscriptionPlanResponseDto createSubscriptionPlan(CreateSubscriptionPlanRequestDto request,
			String adminEmail) {
		...
		auditLogRepository.save(new AdminAuditLog(null, "CREATE_SUBSCRIPTION_PLAN", "SUBSCRIPTION_PLAN",
				createdPlan.getPlanId(), adminEmail != null ? adminEmail : "ADMIN",
