...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
	...
	@Query("SELECT p.merchantId AS merchantId, COALESCE(SUM(p.amount), 0) AS totalRevenue, COUNT(p) AS successfulPayments "
			+ "FROM Payment p WHERE p.status = 'VERIFIED' " + "GROUP BY p.merchantId")
	List<MerchantRevenueProjection> findMerchantRevenueSummary();

	@Query("SELECT p.merchantId AS merchantId, COALESCE(SUM(p.amount), 0) AS totalRevenue, COUNT(p) AS successfulPayments "
			+ "FROM Payment p WHERE p.status = 'VERIFIED' " + "AND (:startDate IS NULL OR p.paymentDate >= :startDate) "
			+ "AND (:endDate IS NULL OR p.paymentDate <= :endDate) " + "GROUP BY p.merchantId")
	List<MerchantRevenueProjection> findMerchantRevenueSummaryByDateRange(@Param("startDate") LocalDateTime startDate,
			@Param("endDate") LocalDateTime endDate);

	@Query("SELECT p.merchantId AS merchantId, COALESCE(SUM(p.amount), 0) AS totalRevenue, COUNT(p) AS successfulPayments "
			+ "FROM Payment p WHERE p.status = 'VERIFIED' AND p.merchantId = :merchantId " + "GROUP BY p.merchantId")
